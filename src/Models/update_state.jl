...
function update_state!(model)
    t = @elapsed update_surface_elevation!(model); println("update surface elevation: ", t)
    t = @elapsed update_geometry_on_uv_grids!(model); println("update uv geometry: ", t)
    t = @elapsed update_height_above_floatation!(model); println("update haf: ", t)
    t = @elapsed update_grounded_fraction_on_huv_grids!(model); println("update grounded fraction: ", t)
    t = @elapsed update_accumulation_rate!(model); println("update accumulation: ", t)
    t = @elapsed update_basal_melt!(model); println("update basal melt: ", t)
    t = @elapsed update_weertman_c!(model); println("update weertman c: ", t)
    t = @elapsed update_dsdh!(model); println("update dsdh: ", t)
    t = @elapsed update_model_velocities!(model); println("update model velocities: ", t)
    t = @elapsed update_velocities_on_h_grid!(model); println("update velocities on h grid: ", t)
    t = @elapsed update_dhdt!(model); println("update dhdt: ", t)
    t = @elapsed update_model_wavelets!(model); println("update wavelets: ", t)
    return nothing
end
