struct QuadraticMeltRateExponentVariationBasins{T <: Real} <: AbstractMeltRate
    γT_basin21 :: T               #(calibrated) heat exchange velocity for basin 21
    γT_basin22 :: T               #(calibrated) heat exchange velocity for basin 22
    λ1 :: T                       #liquidus slope 
...
"""
QuadraticMeltRateExponentVariationBasins(; ,kwargs)

Construct a QuadraticMeltRateExponentVariationBasins object to prescribe the melt rate in WAVI

Keyword arguments 
=================
- γT_basin21: calibrated heat exchange velocity for basin 21 (units m/s).
- γT_basin22: calibrated heat exchange velocity for basin 22 (units m/s).
...
function QuadraticMeltRateExponentVariationBasins(;
                            γT_basin21 = 1.0e-3,
                            γT_basin22 = 1.0e-3,
                            λ1 = -5.73e-2,
                            λ2 = 8.32e-4,
    ...
    return QuadraticMeltRateExponentVariationBasins(γT_basin21, γT_basin22, λ1, λ2, λ3, ρi, ρw, L, c, Ta, Sa, flocal, melt_partial_cell, melt_exp)
end

"""
    update_melt_rate!(quad_melt_rate::QuadraticMeltRateExponentVariationBasins, fields, grid)

Wrapper script to update the melt rate for a QuadraticMeltRateExponentVariationBasins.
"""
function update_melt_rate!(quad_melt_rate::QuadraticMeltRateExponentVariationBasins, fields, grid)
    ...
    set_quadratic_melt_rate_exponent_variation_basins!(basal_melt,
                            quad_melt_rate,
                            zb, 
                            grounded_fraction)
    return nothing
...
function set_quadratic_melt_rate_exponent_variation_basins!(basal_melt,
                                qmr,
                                zb, 
                                grounded_fraction)
...
#    Tstar = qmr.λ1 .* Sa_shelf .+ qmr.λ2 .+ qmr.λ3 .* zb .- Ta_shelf
    ...
    if (qmr.melt_partial_cell) && (qmr.flocal) #partial cell melting and local 
     ...
     basal_melt[grounded_fraction .< 1] .=  qmr.γT_basin21 .* (qmr.ρw * qmr.c / qmr.ρi /qmr.L)^(qmr.melt_exp) .* Tstar[grounded_fraction .< 1].^(qmr.melt_exp)
     basal_melt[:] .=  basal_melt[:] .* (1 .- grounded_fraction[:]) 
  #   basal_melt[grounded_fraction .< 1 && (grid.basin_ID .==22.0)] .=  qmr.γT_basin22 .* (qmr.ρw * qmr.c / qmr.ρi /qmr.L)^(qmr.melt_exp) .* Tstar[grounded_fraction .< 1].^(qmr.melt_exp)
  #   basal_melt[:] .=  basal_melt[:] .* (1 .- grounded_fraction[:]) 
    elseif ~(qmr.melt_partial_cell) && (qmr.flocal) #no partial cell melting and local
        basal_melt[grounded_fraction .== 0  ] .=  qmr.γT_basin21 .* (qmr.ρw * qmr.c / qmr.ρi /qmr.L)^(qmr.melt_exp).* Tstar[grounded_fraction .== 0].^(qmr.melt_exp)
       # basal_melt[grounded_fraction .== 0 && (grid.basin_ID .==22.0) ] .=  qmr.γT_basin22 .* (qmr.ρw * qmr.c / qmr.ρi /qmr.L)^(qmr.melt_exp).* Tstar[grounded_fraction .== 0].^(qmr.melt_exp)
