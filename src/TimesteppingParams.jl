...
function compute_iterations_and_end_time(end_time, n_iter_total, dt)
    ...
    elseif (~(n_iter_total === nothing) && (end_time === nothing)) #if only number of iteractions
    ...
    #check positive numbers, now that both end_time and n_iter_total assigned
    ((isa(n_iter_total, Integer) && (n_iter_total > 0)) || isinf(n_iter_total)) || throw(ArgumentError("n_iter_total must be a positive integer or inf"))
    ((isa(end_time, Real) && (end_time > 0)) || isinf(n_iter_total)) || throw(ArgumentError("end_time must be a positive number or inf"))
